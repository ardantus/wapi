(async function(){
  ...
  function appendMessageToChat(msg){
    ...
    d.innerHTML = `
      ...
      ${contactHtml}
      ${stickerHtml}
      <button class="react-btn" data-msgid="${msgId}" style="font-size:12px;margin-top:4px;padding:2px 8px;cursor:pointer">👍 React</button>
    `;
      ...
      es.addEventListener(evt, e => {
        try {
          ...
          if (evt === 'qr') { logEvent('QR received for '+cid); qrImg.src = '/qr?client='+cid+'&ts='+Date.now(); qrImg.style.display=''; }
            ...
            // refresh chat list (in case new chat appeared or chat order changed)
            loadChats();
  ...
  function renderChats(){
    ...
    filtered.forEach(c => {
      ...
      let picHtml = '';
      if (c.profilePicUrl) {
        picHtml = `<img src="${c.profilePicUrl}" style="width:40px;height:40px;border-radius:50%;margin-right:10px;vertical-align:middle;object-fit:cover" onerror="this.style.display='none'">`;
      } else {
        picHtml = `<div style="width:40px;height:40px;border-radius:50%;margin-right:10px;background:#ddd;display:inline-block;vertical-align:middle"></div>`;
      }
      
      d.innerHTML = picHtml + `<span>${escapeHtml(c.name || c.id) + (c.isGroup ? ' [group]' : '')}</span>`;
